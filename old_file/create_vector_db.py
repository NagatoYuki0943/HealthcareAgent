...
def create_chroma_vectordb(
    ...
    if os.path.exists(persist_directory):
        if not force:
            print(
                f"`{persist_directory}` 路径已存在, 无需创建数据库, 直接读取数据库即可, 如果想强制重新传建, 请设置参数 `force = True`"
            )
            return
        else:
            ...
            print(
                f"\033[0;31;40m`{persist_directory}` 路径已删除,即将重新创建数据库\033[0m"
            )
...
def test_chroma():
    ...
    documents_str, references_str = similarity_search(
        retriever, "Eye Pressure Lowering Effect of Vitamin C"
    )
